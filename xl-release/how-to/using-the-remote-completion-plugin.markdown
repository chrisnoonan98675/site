...
* The user completing or failing a task remotely must send the reply email from the same email address the approval email was send to. This requirement exists for XL Release to be able to authenticate the request. For this reason using email forwarding or email groups or distribution lists might not work.
