...
Using the search pattern from the test tool configuration, the test results files to be imported will be assembled into one list and fed to the test results parser. The output of the parser is a collection of test runs. Each test run is a collection of test results.
...
Where `events` is the list of events generated by the test result parser. `resultHolder.result` is the return value of the script.
