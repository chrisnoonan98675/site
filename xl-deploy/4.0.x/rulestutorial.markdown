...
To run the examples in this tutorial, you need XL Deploy 4.5 or later. No specific configuration or plugin is required.
...
5. Go to the Deployment Workspace and start a new deployment of the application to the environment. When preparing the deployment, click **Deployment Properties** and enter a wait time (if you do not provide a value, the wait step will not appear in the plan).
      ...
      ![Deployment Properties](images/rulestutorial-deployment-properties.png "Deployment Properties")

5. Click **Next**. XL Deploy should create the following deployment plan:

      ![First deployment plan](images/rulestutorial-firstdeployment.png "First deployment plan")
...
![Undeploy plan](images/rulestutorial-undeploy-plan.png "Undeploy plan")
...
The script rule is defined in `xl-rules.xml` as follows:
    ...
    <rule name="example.Server.startStop" scope="plan">
        <planning-script>planning/start-stop-server.py</planning-script>
    </rule>
...
![Final plan](images/rulestutorial-final-plan.png "Final plan")

Notice that steps to start and stop server are added even when application is undeployed:

![Final plan](images/rulestutorial-undeploy-final-plan.png "Final plan")
