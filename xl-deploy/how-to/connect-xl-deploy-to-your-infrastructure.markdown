...
If you would like to use SSH on Windows through WinSSHD or OpenSSH, refer to [Set up SSH in XL Deploy and on a target host](set-up-ssh-in-xl-deploy-and-on-a-target-host.html).
...
8. In the **Password** box, enter the user's password.
      ...
      **Tip:** For information about the permissions that the user must have, see the documentation on [WinRM connections](set-up-winrm-in-xl-deploy-and-on-a-target-host.html).
...
If the connection check fails, refer to tips for troubleshooting [SSH](troubleshoot-an-ssh-connection.html) and [WinRM](troubleshoot-a-winrm-connection.html) connections.
