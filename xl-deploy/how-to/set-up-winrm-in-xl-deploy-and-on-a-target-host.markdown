...
For the WINRM_INTERNAL connection type, domain accounts must be specified using the new-style domain syntax; that is, `USER@FULL.DOMAIN`.

For the WINRM_NATIVE connection type, domain accounts can be specified using either the new-style (`USER@FULL.DOMAIN`) or old-style (`DOMAIN\USER`) domain syntax.

For both connection types, local accounts must be specified without an at-sign (`@`) or a backslash (`\`).

**Note:** When using domain accounts with the WINRM_INTERNAL connection type, the Kerberos subsystem of the Java Virtual Machine must be configured correctly. Please refer to [the information on Kerberos](/xl-deploy/how-to/set-up-kerberos-for-a-winrm-connection.html).
...
For more information about WinRM, refer to [Useful WinRM commands](/xl-deploy/how-to/useful-winrm-commands.html).
