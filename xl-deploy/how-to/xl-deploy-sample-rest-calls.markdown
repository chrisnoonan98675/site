...
The [REST API](https://docs.xebialabs.com/xl-deploy/latest/rest-api/) allows you to execute different commands to create, edit, or delete CIs in XL Deploy. You can access the XL Deploy REST API via a URL of the form: http://[host]:[port]/[context-root]/deployit/[service-resource].
This topic provides examples of various tasks you can perform in XL Deploy using the REST API.
...
**Input**
    ...
    curl -uamy:secret01 -X POST -H "Content-type:application/xml" http://localhost:4516/deployit/repository/ci/Infrastructure/SampleDirectory -d@directory.xml
...
**Input**
    ...
    curl -uamy:secret01 -X POST -H "Content-type:application/xml" http://localhost:4516/deployit/repository/ci/Infrastructure/SampleDirectory/SampleSSHHost -d@ssh-host.xml
...
**Input**
	...
	curl -uamy:secret01 -X POST -H "Content-type:application/xml" http://localhost:4516/deployit/repository/ci/Infrastructure/SampleDirectory/SampleSSHHost/SampleTomcatServer -d@tomcat-server.xml
...
**Input**
	...
	curl -uamy:secret01 -X POST -H "Content-type:application/xml" http://localhost:4516/deployit/repository/ci/Infrastructure/SampleDirectory/SampleSSHHost/SampleTomcatServer/SampleVirtualHost -d@tomcat-virtual-host.xml
...
If the environment does not exist:
    ...
    curl -uamy:secret01 -X POST -H "Content-type:application/xml" http://localhost:4516/deployit/repository/ci/Environments/TestEnv -d@environment.xml

If the environment already exists and is called _TestEnv_:

    curl -uamy:secret01 -X PUT -H "Content-type:application/xml" http://localhost:4516/deployit/repository/ci/Environments/TestEnv -d@environment.xml
...
**Input**
    ...
    curl -uamy:secret01 -X DELETE -H "Content-type:application/xml" http://localhost:4516/deployit/repository/ci/Infrastructure/SampleDirectory/SampleSSHHost/SampleTomcatServer/SampleVirtualHost
